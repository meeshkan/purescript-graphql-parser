module Test.Data.GraphQL.RetrieveStringTypes where
...
getFieldNamesFromDocument ∷ AST.Document → List String
getFieldNamesFromDocument =
  L.toListOf $ uncurry L.prism' AST._Document
    <<< L.traversed
    ...
    <<< L.filtered
        ( \fd ->
            fd.type == (AST.Type_NamedType $ AST.NamedType "String")
              || fd.type
              == ( AST.Type_NonNullType
                    (AST.NonNullType_NamedType (AST.NamedType "String"))
                )
        )
    <<< LR.prop (SProxy :: SProxy "name")

retrieveStringTypes ∷ ∀ m. Monad m ⇒ SpecT Aff Unit m Unit
retrieveStringTypes =
  describe "test string type retriever" do
    before (parseDocument query)
      $ do
          it "should retrieve only string types" \doc → do
            Set.subset
              (Set.fromFoldable [ "username", "name" ])
              (Set.fromFoldable $ getFieldNamesFromDocument doc)
              `shouldEqual`
                true
            Set.subset
              (Set.fromFoldable [ "username", "fhufweuifhwe" ])
              (Set.fromFoldable $ getFieldNamesFromDocument doc)
              `shouldEqual`
                false
            Set.subset
              (Set.fromFoldable [ "username", "published" ])
              (Set.fromFoldable $ getFieldNamesFromDocument doc)
              `shouldEqual`
                false
